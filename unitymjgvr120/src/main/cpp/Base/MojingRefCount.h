#ifndef MJ_RefCount_h
#define MJ_RefCount_h
...
namespace Baofeng
{
	namespace Mojing
	{
		...
		public:
			...
			MJ_FORCE_INLINE RefCountImplCore() : RefCount(1) { }
			...
			static void   MJ_CDECL  reportInvalidDelete(void *pmem);
		...
		public:
			// RefCountImpl constructor always initializes RefCount to 1 by default.
			MJ_FORCE_INLINE RefCountNTSImplCore() : RefCount(1) { }
...
#ifdef MJ_BUILD_DEBUG    
			static void   MJ_CDECL  reportInvalidDelete(void *pmem);
			MJ_FORCE_INLINE static void checkInvalidDelete(RefCountNTSImplCore *pmem)
			{
				if (pmem->RefCount != 0)
					reportInvalidDelete(pmem);
			}
#else
			MJ_FORCE_INLINE static void checkInvalidDelete(RefCountNTSImplCore *) { }
#endif
		...
		{
		public:
			MJ_FORCE_INLINE void    AddRef() const { RefCount++; }
			void    Release() const;
		};
...
#define MJ_REFCOUNTALLOC_CHECK_DELETE(class_name, p)   \
...
#define MJ_REFCOUNTALLOC_CHECK_DELETE(class_name, p)
			...
			MJ_MEMORY_REDEFINE_NEW_IMPL(Base, MJ_REFCOUNTALLOC_CHECK_DELETE)
...
#define new MJ_DEFINE_NEW
		...
		public:
			// Constructor.
			MJ_FORCE_INLINE RefCountBase() : RefCountBaseStatImpl<RefCountImpl>() { }
		};
		...
		public:
			// Constructor.
			MJ_FORCE_INLINE RefCountBaseV() : RefCountBaseStatImpl<RefCountVImpl>() { }
		};
		...
		public:
			// Constructor.
			MJ_FORCE_INLINE RefCountBaseNTS() : RefCountBaseStatImpl<RefCountNTSImpl>() { }
		};
		...
		public:
			...
			// Constructors
			MJ_FORCE_INLINE Ptr() : pObject(0)
			{ }
#ifdef MJ_CC_ARM
			MJ_FORCE_INLINE Ptr(C &robj) : pObject(ReturnArg(&robj))
#else
			MJ_FORCE_INLINE Ptr(C &robj) : pObject(&robj)
#endif
			{ }
			MJ_FORCE_INLINE Ptr(Pickable<C> v) : pObject(v.GetPtr())
			{
				// No AddRef() on purpose.
			}
			MJ_FORCE_INLINE Ptr(Ptr<C>& other, PickType) : pObject(other.pObject)
			{
				other.pObject = NULL;
				// No AddRef() on purpose.
			}
			MJ_FORCE_INLINE Ptr(C *pobj)
			{
				if (pobj) pobj->AddRef();
				pObject = pobj;
			}
			MJ_FORCE_INLINE Ptr(const Ptr<C> &src)
			{
			...
			template<class R>
			MJ_FORCE_INLINE Ptr(Ptr<R> &src)
			{
				if (src) src->AddRef();
				pObject = src;
			}
			template<class R>
			MJ_FORCE_INLINE Ptr(Pickable<R> v) : pObject(v.GetPtr())
			{
				// No AddRef() on purpose.
			}
			...
			// Compares
			MJ_FORCE_INLINE bool operator == (const Ptr &other) const       { return pObject == other.pObject; }
			MJ_FORCE_INLINE bool operator != (const Ptr &other) const       { return pObject != other.pObject; }

			MJ_FORCE_INLINE bool operator == (C *pother) const              { return pObject == pother; }
			MJ_FORCE_INLINE bool operator != (C *pother) const              { return pObject != pother; }
			...
			// Assignment
			template<class R>
			MJ_FORCE_INLINE const Ptr<C>& operator = (const Ptr<R> &src)
			{
				if (src) src->AddRef();
				if (pObject) pObject->Release();
				pObject = src;
			...
			// Specialization
			MJ_FORCE_INLINE const Ptr<C>& operator = (const Ptr<C> &src)
			{
				if (src) src->AddRef();
				if (pObject) pObject->Release();
				pObject = src;
			...
			MJ_FORCE_INLINE const Ptr<C>& operator = (C *psrc)
			{
				if (psrc) psrc->AddRef();
				if (pObject) pObject->Release();
				pObject = psrc;
			...
			MJ_FORCE_INLINE const Ptr<C>& operator = (C &src)
			{
				if (pObject) pObject->Release();
				pObject = &src;
				return *this;
			}
			MJ_FORCE_INLINE Ptr<C>& operator = (Pickable<C> src)
			{
				return Pick(src);
			}
			template<class R>
			MJ_FORCE_INLINE Ptr<C>& operator = (Pickable<R> src)
			{
				return Pick(src);
			}
			...
			// Set Assignment
			template<class R>
			MJ_FORCE_INLINE Ptr<C>& SetPtr(const Ptr<R> &src)
			{
				if (src) src->AddRef();
				if (pObject) pObject->Release();
				pObject = src;
			...
			// Specialization
			MJ_FORCE_INLINE Ptr<C>& SetPtr(const Ptr<C> &src)
			{
				if (src) src->AddRef();
				if (pObject) pObject->Release();
				pObject = src;
			...
			MJ_FORCE_INLINE Ptr<C>& SetPtr(C *psrc)
			{
				if (psrc) psrc->AddRef();
				if (pObject) pObject->Release();
				pObject = psrc;
			...
			MJ_FORCE_INLINE Ptr<C>& SetPtr(C &src)
			{
				if (pObject) pObject->Release();
				pObject = &src;
				return *this;
			}
			MJ_FORCE_INLINE Ptr<C>& SetPtr(Pickable<C> src)
			{
				return Pick(src);
			}

			// Nulls ref-counted pointer without decrement
			MJ_FORCE_INLINE void    NullWithoutRelease()
			{
				pObject = 0;
			}

			// Clears the pointer to the object
			MJ_FORCE_INLINE void    Clear()
			{
				if (pObject) pObject->Release();
				pObject = 0;
			}
			...
			MJ_FORCE_INLINE C*& GetRawRef()                 { return pObject; }

			// Access Operators
			MJ_FORCE_INLINE C* GetPtr() const               { return pObject; }
			MJ_FORCE_INLINE C& operator * () const          { return *pObject; }
			MJ_FORCE_INLINE C* operator -> ()  const        { return pObject; }
			// Conversion                   
			MJ_FORCE_INLINE operator C* () const            { return pObject; }

			// Pickers.

			// Pick a value.
			MJ_FORCE_INLINE Ptr<C>& Pick(Ptr<C>& other)
			{
				if (&other != this)
				{
					if (pObject) pObject->Release();
			...
			MJ_FORCE_INLINE Ptr<C>& Pick(Pickable<C> v)
			{
				if (v.GetPtr() != pObject)
				{
					if (pObject) pObject->Release();
			...
			template<class R>
			MJ_FORCE_INLINE Ptr<C>& Pick(Pickable<R> v)
			{
				if (v.GetPtr() != pObject)
				{
					if (pObject) pObject->Release();
			...
			MJ_FORCE_INLINE Ptr<C>& Pick(C* p)
			{
				if (p != pObject)
				{
					if (pObject) pObject->Release();
