...
{// 清除所有未更新设备的记录
	...
	for (DeviceMap::iterator it = g_AllDeviceMap.begin(); it != g_AllDeviceMap.end(); it++)
	{
		...
		if ((MID_JOYSTICK_MJ2_MAYBE == it->second.m_DeviceType) && 1 == g_Mojing2Number)
		{
...
JNIEXPORT void JNICALL Java_com_baofeng_mojing_MojingSDK_NativeAddDeviceToMap(JNIEnv *env, jclass, jint jiDeviceID, jstring jsDeviceName)
{
	...
	else if (0 == strcmp("Broadcom Bluetooth HID", szDeviceName) || 0 == strcmp("MTK BT HID", szDeviceName))
	{
		g_AllDeviceMap[jiDeviceID].m_DeviceType = MID_JOYSTICK_MJ2_MAYBE;
	}
	else
	{
