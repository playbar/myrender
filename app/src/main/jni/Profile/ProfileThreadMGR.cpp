...
#else
				strRet = "http://fuwu.mojing.cn/api/sdk/getjoystickconfig";
...
#else
				strRet = "http://fuwu.mojing.cn/api/sdk/getglassconfig";
...
//#endif
#else
				strRet = "http://mjsdk.log.mojing.cn/logger.php";
#endif
			...
			{
				strURL = "http://open.mojing.cn/api/developer/merchantverify";
			...
			else
			{
				// 这里是原始的老戴的服务器
#ifdef _DEBUG
				strURL = "http://192.168.12.61:8887/v1.0/";
				MOJING_TRACE(g_APIlogger, "Profile using inside address." << (IO == Profile_LOAD ? "Load..." : "Post..."));
#else
				// strURL = "http://api.mojing.baofeng.com/mj/v1.0/";
				strURL = "http://api.platform.mojing.cn/mj/v1.0/";// <-- 停用老的服务器
				...
				if (szClassName == "GyroTempCalibrationReporter")
				{
					if (IO == Profile_LOAD)
					{
						strRet = strURL + "mjswitchconfig.php";
					}
					else// IO == Profile_SAVE
					{
						strRet = strURL + "mjuploadconfig.php";
					}
				}
				else if (szClassName == "MobileConfigUpdate")
				{
					if (IO == Profile_LOAD)
					{
						strRet = strURL + "mjgetgyroscopeconfig.php";
					}
				}
				else if (szClassName == "CrashReporter")
				{
					if (IO == Profile_SAVE)
					{
						strRet = strURL + "mjsdkuploadcrash.php";
					}
				}
			}
			return strRet;
