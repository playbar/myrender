...
//
+ (NSString*) value {
    return [MJOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [MJOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [MJOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [MJOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
