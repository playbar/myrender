	...
	namespace Mojing
	{
		...
		protected:
			...
			inline void        SetData(DataDesc* pdesc)
			{
				...
				pData = pdesc;
				MJ_ASSERT((HeapTypeBits & HT_Mask) == 0);
				HeapTypeBits |= ht;
			}
		...
		public:
			...
			static int MJ_STDCALL   CompareNoCase(const char* a, const char* b);
			static int MJ_STDCALL   CompareNoCase(const char* a, const char* b, SPInt len);

			// Hash function, case-insensitive
			static UPInt MJ_STDCALL BernsteinHashFunctionCIS(const void* pdataIn, UPInt size, UPInt seed = 5381);

			// Hash function, case-sensitive
			static UPInt MJ_STDCALL BernsteinHashFunction(const void* pdataIn, UPInt size, UPInt seed = 5381);


			// ***** File path parsing helper functions.
			// Implemented in MJ_String_FilePath.cpp.
			...
			{
				return (MJ_strcmp(GetData()->Data, str.GetData()->Data) == 0);
			}
			...
			{
				return MJ_strcmp(GetData()->Data, str) == 0;
			}
			...
			{
				return MJ_strcmp(GetData()->Data, pstr) < 0;
			}
		...
		public:
			...
			StringDataPtr(const char* pstr)
				: pStr(pstr), Size((pstr != NULL) ? MJ_strlen(pstr) : 0) {}
		...
		public:
			...
			{
				return (MJ_strncmp(pStr, data.pStr, data.Size) == 0);
			}
