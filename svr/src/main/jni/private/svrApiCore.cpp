...
{
	ReleaseVertexTable();
	gVertexTable = new VertexTable;
	gVertexTable->m_iVertexSize = VertexSize / 8 * 10;
	float fAppend[2] = {0,0};
	gVertexTable->m_pVertexLeft = VertexAttribute8To10(pVertexL, VertexSize, fAppend);

	fAppend[0] = 1;
	gVertexTable->m_pVertexRight = VertexAttribute8To10(pVertexR, VertexSize, fAppend);

	gVertexTable->m_iIndexSize = IndexSize;
	gVertexTable->m_pIndex = new unsigned int[IndexSize];
	memcpy(gVertexTable->m_pIndex , pIndex , IndexSize * sizeof(unsigned int));
