...
#define String_LengthIsSize (UPInt(1) << String::Flag_LengthIsSizeShift)
		...
		String::String(const char* pdata)
		{
			...
			UPInt size = pdata ? MJ_strlen(pdata) : 0;
		...
		{
			// Obtain length in bytes; it doesn't matter if _data is UTF8.
			UPInt size1 = pdata1 ? MJ_strlen(pdata1) : 0;
			UPInt size2 = pdata2 ? MJ_strlen(pdata2) : 0;
			UPInt size3 = pdata3 ? MJ_strlen(pdata3) : 0;
		...
		String::DataDesc* String::AllocData(UPInt size, UPInt lengthIsSize)
		{
			...
			pdesc = (DataDesc*)MJ_ALLOC(sizeof(DataDesc)+size);
			pdesc->Data[size] = 0;
			pdesc->RefCount = 1;
		...
		void String::AppendString(const char* putf8str, SPInt utf8StrSz)
		{
			if (!putf8str || !utf8StrSz)
				return;
			if (utf8StrSz == -1)
				utf8StrSz = (SPInt)MJ_strlen(putf8str);
		...
		{
			AssignString(pstr, pstr ? MJ_strlen(pstr) : 0);
		}
		...
		String   String::ToUpper() const
		{
			...
			while (psource < pend)
			{
				do {
					...
					UTF8Util::EncodeChar(buffer, &bufferOffset, MJ_towupper(wchar_t(c)));
		...
		String   String::ToLower() const
		{
			...
			while (psource < pend)
			{
				do {
					c = UTF8Util::DecodeNextChar_Advance0(&psource);
					UTF8Util::EncodeChar(buffer, &bufferOffset, MJ_towlower(wchar_t(c)));
				} while ((psource < pend) && (bufferOffset < SPInt(sizeof(buffer)-8)));
		...
		*/
		...
		int String::CompareNoCase(const char* a, const char* b, SPInt len)
		{
			if (len)
			{
				...
				const char *s = b;
				do {
					f = (SPInt)MJ_tolower((int)(*(a++)));
					l = (SPInt)MJ_tolower((int)(*(b++)));
				} while (--len && f && (f == l) && *b != 0);
...
#define MJ_SBUFF_DEFAULT_GROW_SIZE 512
		// Constructors / Destructor.
		StringBuffer::StringBuffer()
			: pData(NULL), Size(0), BufferSize(0), GrowSize(MJ_SBUFF_DEFAULT_GROW_SIZE), LengthIsSize(false)
		{
		}
		...
		void    StringBuffer::Reserve(UPInt _size)
		{
			if (_size >= BufferSize) // >= because of trailing zero! (!AB)
			{
				...
				if (!pData)
					pData = (char*)MJ_ALLOC(BufferSize);
				else
					pData = (char*)MJ_REALLOC(pData, BufferSize);
			}
		}
		...
		{
			UPInt     oldSize = Size;
			UPInt     insertSize = (len < 0) ? MJ_strlen(substr) : (UPInt)len;
