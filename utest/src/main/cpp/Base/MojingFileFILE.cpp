...
#include "windows.h"
...
#ifdef MJ_FILE_VERIFY_SEEK_ERRORS
		...
		void FILEFile::init()
		{
			...
			wchar_t womode[16];
			wchar_t *pwFileName = (wchar_t*)MJ_ALLOC((UTF8Util::GetLength(FileName.ToCStr()) + 1) * sizeof(wchar_t));
			UTF8Util::DecodeString(pwFileName, FileName.ToCStr());
			MJ_ASSERT(strlen(omode) < sizeof(womode) / sizeof(womode[0]));
			...
			else
			{
				...
				fseek(fs, 0, SEEK_SET);
				pFileTestBuffer = (UByte*)MJ_ALLOC(FileTestLength);
				if (pFileTestBuffer)
				{
					MJ_ASSERT(FileTestLength == (unsigned)Read(pFileTestBuffer, FileTestLength));
					Seek(0, Seek_Set);
				}        
#endif
		...
		{
		return IsValid() && ((OpenFlags&MJ_FO_SAFETRUNC) == MJ_FO_SAFETRUNC);
		}
		*/
		...
		int     FILEFile::Read(UByte *pbuffer, int numBytes)
		{
			...
			if (read > 0)
			{
				...
				{
					MJ_ASSERT(pcompareBuffer[i] == pbuffer[i]);
				}

				//MJ_ASSERT(!memcmp(pFileTestBuffer + TestPos, pbuffer, read));
				TestPos += read;
				MJ_ASSERT(ftell(fs) == (int)TestPos);
			}
#endif
		...
		bool FILEFile::Close()
		{
			...
			/*
			if ((OpenFlags & MJ_FO_SAFETRUNC) == MJ_FO_SAFETRUNC)
			{
		...
		*/
		...
		bool    SysFile::GetFileStat(FileStat* pfileStat, const String& path)
		{
			...
			wchar_t *pwpath = (wchar_t*)MJ_ALLOC((UTF8Util::GetLength(path.ToCStr()) + 1)*sizeof(wchar_t));
			UTF8Util::DecodeString(pwpath, path.ToCStr());

			int ret = _wstat(pwpath, &fileStat);
			MJ_FREE(pwpath);
			if (ret) return false;
#else
