#ifndef MJ_UTF8Util_h
#define MJ_UTF8Util_h
...
SPInt    MJ_STDCALL GetLength(const char* putf8str, SPInt length = -1);
...
UInt32   MJ_STDCALL GetCharAt(SPInt index, const char* putf8str, SPInt length = -1);

// Converts UTF8 character index into byte offset.
// -1 is returned if index was out of bounds.
SPInt    MJ_STDCALL GetByteIndex(SPInt index, const char* putf8str, SPInt length = -1);
...
SPInt    MJ_STDCALL GetEncodeStringSize(const wchar_t* pchar, SPInt length = -1);

// Encodes a unicode (UCS-2 only) string into a buffer. The size of buffer must be at
// least GetEncodeStringSize() + 1.
void     MJ_STDCALL EncodeString(char *pbuff, const wchar_t* pchar, SPInt length = -1);
...
UPInt    MJ_STDCALL DecodeString(wchar_t *pbuff, const char* putf8str, SPInt bytesLen = -1);


// *** Individual character Encoding/Decoding.

// Determined the number of bytes necessary to encode a UCS character.
int      MJ_STDCALL GetEncodeCharSize(UInt32 ucsCharacter);
...
void     MJ_STDCALL EncodeChar(char* pbuffer, SPInt* poffset, UInt32 ucsCharacter);
