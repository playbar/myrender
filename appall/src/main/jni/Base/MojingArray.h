	...
	namespace Mojing
	{
		...
		struct ArrayDataBase
		{
			...
			void Reserve(UPInt newCapacity)
			{
				...
				else
				{
					...
					// MJ_ASSERT(Data); // need to throw (or something) on alloc failure!
				}
			}
		...
		struct ArrayDataCC : ArrayDataBase<T, Allocator, SizePolicy>
		{
			...
			ArrayDataCC(const SelfType& a)
				: BaseType(a.Policy), DefaultValue(a.DefaultValue) {
				Append(a.Data, a.Size);
			}
...
#undef new
			MJ_MEMORY_REDEFINE_NEW(ArrayBase)
...
#define new MJ_DEFINE_NEW
			...
			void    PushBack(const ValueType& val)
			{
				...
				// MJ_ASSERT(&val < &Buffer[0] || &val > &Buffer[BufferSize]);
				Data.PushBack(val);
			}
			...
			public:
				...
				{
					MJ_ASSERT(pArray == right.pArray);
					return CurIndex - right.CurIndex;
				}
				ValueType& operator*() const    { MJ_ASSERT(pArray); return  (*pArray)[CurIndex]; }
				ValueType* operator->() const   { MJ_ASSERT(pArray); return &(*pArray)[CurIndex]; }
				ValueType* GetPtr() const       { MJ_ASSERT(pArray); return &(*pArray)[CurIndex]; }
			...
			public:
				...
				const ValueType& operator*() const  { MJ_ASSERT(pArray); return  (*pArray)[CurIndex]; }
				const ValueType* operator->() const { MJ_ASSERT(pArray); return &(*pArray)[CurIndex]; }
				const ValueType* GetPtr() const     { MJ_ASSERT(pArray); return &(*pArray)[CurIndex]; }
