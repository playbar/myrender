...
namespace Baofeng
{
	namespace Mojing
	{
		...
		protected:
			...
			DelegatedFile(const DelegatedFile &source) : File()    { MJ_UNUSED(source); }
		public:
			// Constructors
		...
		protected:
			...
			// Hidden constructor
			BufferedFile();
			inline BufferedFile(const BufferedFile &source) : DelegatedFile() { MJ_UNUSED(source); }
		public:
		...
		public:
			...
			{
				MJ_UNUSED2(pstream, byteSize);
				return 0;
			}

			int         Write(const UByte *pbuffer, int numBytes)
			{
				MJ_UNUSED2(pbuffer, numBytes);
				return 0;
			}
		...
		bool MakeDirs(const char* sDirs, int mod = 0777);// 0777 是8进制数组
	}
} // OVR
