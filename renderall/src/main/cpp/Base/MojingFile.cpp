...
#define FILEBUFFER_TOLERANCE    4096
			...
			{
				pBuffer = (UByte*)MJ_ALLOC(FILEBUFFER_SIZE);
				BufferMode = NoBuffer;
				FilePos = 0;
				Pos = 0;
				DataSize = 0;
			...
			*/
			...
			int     BufferedFile::Tell()
			{
				...
				if (pos != -1)
				{
					MJ_ASSERT(BufferMode != ReadBuffer);
			...
			*/
			...
			int     BufferedFile::Seek(int offset, int origin)
			{
				if (BufferMode == ReadBuffer)
				{
					if (origin == Seek_Cur)
					{
						...
						origin = Seek_Set;
						MJ_ASSERT(((FilePos - DataSize + Pos) + (UInt64)offset) < ~(UInt64)0);
					...
					else if (origin == Seek_Set)
					{
						...
						{
							MJ_ASSERT((FilePos - DataSize) < ~(UInt64)0);
				...
				{
				//MJ_ASSERT((FilePos - DataSize) >= (FilePos - DataSize + Pos + offset));
				Pos += offset;
				MJ_ASSERT(int (Pos) >= 0);
				return int (FilePos - DataSize + Pos);
				}
			...
			const char* MJ_CDECL GetShortFilename(const char* purl)
			{
				UPInt len = MJ_strlen(purl);
			...
			bool MakeDirs(const char* sDirs, int mod /*= 0777*/)
			{
				char *pCurrent = new char[strlen(sDirs) + 1];
				strcpy(pCurrent , sDirs);
				int iLen = (int)strlen(pCurrent) - 1;
				// 去掉末尾的'/'和'\'
				while (iLen >= 0 && (pCurrent[iLen] == '/' || pCurrent[iLen] == '\\'))
				{
					pCurrent[iLen] = 0;
					iLen--;
				}
				if (iLen < 0 || *pCurrent == 0)
				{
					delete[] pCurrent;
					return false;
				}
				DIR* curDir = opendir(pCurrent);
				if (curDir)
				{
					closedir(curDir);
					delete[] pCurrent;
				...
				// 创建上层目录
				char * pParent = new char[strlen(pCurrent)+1];
				strcpy(pParent  , pCurrent);
				while (iLen >= 0 && (pParent[iLen] != '/' && pParent[iLen] != '\\'))
				{
					pParent[iLen] = 0;
					iLen--;
				}
...
#ifdef MJ_OS_WIN32
				bool bRet = MakeDirs(pParent, mod) && 0 == mkdir(pCurrent);
#else
				bool bRet = MakeDirs(pParent, mod) && 0 == mkdir(pCurrent, mod);
#endif
				delete[] pParent;
				delete[] pCurrent;
				return bRet;
