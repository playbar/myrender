...
namespace Baofeng
{
	namespace Mojing
	{
		...
		public:
			...
			virtual int         Write(const UByte *pbuffer, int numBytes)     { return -1; MJ_UNUSED2(pbuffer, numBytes); }
			virtual int         Read(UByte *pbuffer, int numBytes)            { return -1; MJ_UNUSED2(pbuffer, numBytes); }
			virtual int         SkipBytes(int numBytes)                       { return 0;  MJ_UNUSED(numBytes); }
			virtual int         BytesAvailable()                              { return 0; }
			virtual bool        Flush()                                       { return 0; }
			virtual int         Seek(int offset, int origin)                  { return -1; MJ_UNUSED2(offset, origin); }
			virtual SInt64      LSeek(SInt64 offset, int origin)              { return -1; MJ_UNUSED2(offset, origin); }

			virtual int         CopyFromStream(File *pstream, int byteSize)   { return -1; MJ_UNUSED2(pstream, byteSize); }
		...
		bool SysFile::Open(const String& path, int flags, int mode)
		{
			...
			//pFile = *MJ_NEW DelegatedFile(pFile); // MA Testing
