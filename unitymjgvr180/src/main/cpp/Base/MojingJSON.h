...
namespace Baofeng
{
	namespace Mojing
	{
		...
		public:
			...
			static JSON*    ParseEnc(const unsigned char* buff, const int len, unsigned char *pKey, const char** perror = 0);

			// Loads and parses a JSON object from a file.
			// Returns a null pointer and fills in *perror in case of parse error.
			static JSON*    Load(const char* path, unsigned char *pKey = NULL , const char** perror = 0);
			// Saves a JSON object to a file.
			bool            Save(const char* path, unsigned char *pKey = NULL);
			...
			JSON*			Clon(){ char *pTemp = PrintValue(0, false); JSON*pRet = JSON::Parse(pTemp); MJ_FREE(pTemp); return pRet; };
		...
		public:
			...
			bool			IsEndOfArray() const { MJ_ASSERT(Parent != NULL); return Parent->Children.IsNull(Child); }
		...
		/* 工具函数                                                             */
		/************************************************************************/
		typedef bool(*FP_CheckJsonNode) (JSON *pNode, void *pParam);
		JSON * JSON_SelectNode(JSON * pJsonRoot, const char* lpszClassName, int iMaxVersion, FP_CheckJsonNode fpCheck, void *pParam);
		JSON * JSON_CompReleaseData(JSON *pA, JSON *pB, char *szOutReleaseDate = NULL);
