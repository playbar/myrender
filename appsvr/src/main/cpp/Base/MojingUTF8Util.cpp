...
SPInt MJ_STDCALL GetLength(const char* buf, SPInt buflen)
{
    const char* p = buf;
    SPInt length = 0;
...
UInt32 MJ_STDCALL GetCharAt(SPInt index, const char* putf8str, SPInt length)
{
...
SPInt MJ_STDCALL GetByteIndex(SPInt index, const char *putf8str, SPInt length)
{
    const char* buf = putf8str;
...
int MJ_STDCALL GetEncodeCharSize(UInt32 ucs_character)
{
...
UInt32 MJ_STDCALL DecodeNextChar_Advance0(const char** putf8Buffer)
{
    UInt32  uc;
    char    c;
...
void MJ_STDCALL EncodeChar(char* pbuffer, SPInt* pindex, UInt32 ucs_character)
{
    if (ucs_character <= 0x7F)
    {
...
SPInt MJ_STDCALL GetEncodeStringSize(const wchar_t* pchar, SPInt length)
{
    SPInt len = 0;
    if (length != -1)
...
void MJ_STDCALL EncodeString(char *pbuff, const wchar_t* pchar, SPInt length)
{
    SPInt ofs = 0;
    if (length != -1)
    {
...
UPInt MJ_STDCALL DecodeString(wchar_t *pbuff, const char* putf8str, SPInt bytesLen)
{
...
bool    check_equal(const char* utf8_in, const UInt32* ucs_in)
{
    for (;;)
    {
        ...
        if (next_ucs == 0)
        {
            MJ_ASSERT(next_ucs_from_utf8 == 0);
            break;
        }
    }
...
{
    {
        ...
        MJ_ASSERT(check_equal(test8, test32));
    }
                ...
                if (c == '\n' || byte_counter >= LINE_SIZE - 2)
                {
                    ...
                    for (;;)
                    {
                        ...
                        *q++ = uc;
                        ...
                        MJ_ASSERT(q < line_buffer_ucs + LINE_SIZE);
                        MJ_ASSERT(p < line_buffer_utf8 + LINE_SIZE);
                    ...
                    {
                        UInt32  uc = *q++;
                        MJ_ASSERT(index < LINE_SIZE * 6 - 6);
                        int last_index = index;
                        UTF8Util::EncodeChar(reencoded_utf8, &index, uc);
                        MJ_ASSERT(index <= last_index + 6);
                        if (uc == 0) break;
                    }
                    ...
                    MJ_ASSERT(check_equal(line_buffer_utf8, line_buffer_ucs));
                    MJ_ASSERT(check_equal(reencoded_utf8, line_buffer_ucs));
