...
#include "MojingContainerAllocator.h"
#include "MojingAlg.h"
...
public:
    ...
    static MJ_FORCE_INLINE UPInt SDBM_Hash(const void* data_in, UPInt size, UPInt seed = 5381)     
    {
...
public:
    ...
    // Cached hash value access - can be optimized bu storing hash locally.
    // Mask value only needs to be used if SetCachedHash is not implemented.
    UPInt   GetCachedHash(UPInt maskValue) const  { MJ_UNUSED(maskValue); return HashValue; }
...
public:
    MJ_MEMORY_REDEFINE_NEW(HashSetBase)
...
# pragma warning(disable : 4284)
    ...
    {   
        ...
        {            
            MJ_ASSERT(Index >= 0 && Index <= (SPInt)pHash->pTable->SizeMask);
    ...
    {      
        ...
        C&  operator*() const
        {            
            MJ_ASSERT(ConstIterator::Index >= 0 && ConstIterator::Index <= (SPInt)ConstIterator::pHash->pTable->SizeMask);
...
private:
    ...
    SPInt findIndexCore(const K& key, UPInt hashValue) const
    {
        ...
        MJ_ASSERT((hashValue & ~pTable->SizeMask) == 0);
        ...
        while(1)
        {
            MJ_ASSERT(e->GetCachedHash(pTable->SizeMask) == hashValue);
    ...
    void add(const CRef& key, UPInt hashValue)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                MJ_ASSERT(collidedIndex >= 0 && collidedIndex <= (SPInt)pTable->SizeMask);
                for (;;)
                {
    ...
    Entry& E(UPInt index)
    {
        ...
        MJ_ASSERT(index <= pTable->SizeMask);
    ...
    void    setRawCapacity(UPInt newSize)    
    {
        ...
        else
        {
            ...
            MJ_ASSERT((UPInt(1) << bits) >= newSize);
...
public:
    ...
    bool    IsEndOfChain() const        { return NextInChain == -1;  }
    UPInt   GetCachedHash(UPInt maskValue) const  { return HashF()(Value) & maskValue; }
    void    SetCachedHash(UPInt hashValue)        { MJ_UNUSED(hashValue); }
